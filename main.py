...
if selected_mbti in mbti_matches:
    ...
    for place in match['places']:
        ...
        # 위치 데이터가 없을 경우 생략됨 (위도/경도 데이터는 실제로 사용 시 추가 필요)
        # 예시로 하나의 고정된 위치 사용
        map_view = folium.Map(location=[37.5665, 126.9780], zoom_start=12)
        folium.Marker([37.5665, 126.9780], popup=place).add_to(map_view)
